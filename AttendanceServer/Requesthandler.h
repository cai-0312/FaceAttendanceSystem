...
/**
 * @brief 网络请求处理器
 *        将所有客户端请求按业务分组，拆分为独立的静态处理函数。
 *        每个函数运行在 QtConcurrent 线程中，使用独立的线程数据库连接。
 *
 *  调用约定：
 *    - threadDb  ：调用方已打开的线程级 ODBC 连接
 *    - socket    ：发起请求的客户端套接字（回包时通过 QMetaObject::invokeMethod 切回主线程）
 *    - json      ：已解析的请求 JSON 对象
 *    - 需要访问在线用户表的处理器接受 server 指针，以便通过 invokeMethod 回调主线程
 */
