...
void FaceProcessThread::requestAiGreeting(QString name)
{
    ...
    // ⭐️ 核心修改 1：将云端 API 换成 Ollama 的本地服务端口
    QUrl url("http://127.0.0.1:11434/v1/chat/completions");
    QNetworkRequest request(url);
    request.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    // 本地调用不需要 API Key，直接删掉鉴权头！

    QJsonObject json;
    // ⭐️ 核心修改 2：指定为你下载的本地模型名称
    ...
    sysMsg["role"] = "system";
    sysMsg["content"] = "你是一个企业考勤打卡机内置的语音助手。请根据员工姓名、打卡时间和天气，生成一句热情、极其简短的打卡成功欢迎语（控制在15个字以内，不要输出其他废话）。";
    userMsg["role"] = "user";
